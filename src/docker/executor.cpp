...
int main(int argc, char** argv)
{
  ...
  chameleon::logging::initialize(argv[0], flags, true); // Catch signals.
  ...
  LOG(INFO) <<"Heldon docker-executor flags" <<stringify(flags);
  if (flags.help) {
  ...
  Duration shutdownGracePeriod = Seconds(5);
...
//  }
  ...
  Owned<chameleon::docker::DockerExecutor> executor(
      new chameleon::docker::DockerExecutor(
          docker.get(),
          flags.container.get(),
